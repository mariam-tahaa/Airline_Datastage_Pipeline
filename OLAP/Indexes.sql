...
CREATE INDEX IDX_FACT_TICKET_FARE ON FactTicket (FARE_CLASS_SK);
...
CREATE INDEX IDX_RES_CHANNEL ON FACT_RESERVATION_TABLES (Channel_Key);

CREATE INDEX IDX_RES_FARE ON FACT_RESERVATION_TABLES (Fare_Key);
...
CREATE INDEX IDX_INTERACTION_TYPE ON FACT_CUSTOMER_INTERACTION (INTERACTION_TYPE);

CREATE INDEX IDX_INTERACTION_SEVERITY ON FACT_CUSTOMER_INTERACTION (SEVERITY);
...
CREATE INDEX IDX_UPGRADE_CHANNEL ON FACT_FLIGHT_UPGRADE (CHANNEL_KEY);
